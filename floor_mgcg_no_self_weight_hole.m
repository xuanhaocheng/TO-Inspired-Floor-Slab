...
floor_thickness = round(0.03 / 0.2 * nelz);
pasS1 = double(reshape(elmNrs(1:nely,1:floor_thickness, 1: nelx),[],1));
pasS2 = double(reshape(elmNrs(1:nely,nelz -floor_thickness+1:nelz, 1: nelx),[],1));
pasS = union(pasS1, pasS2);
hole_width = round(0.5 / 4.8 * nelx);
pasV = double(reshape(elmNrs(nely -hole_width+1:nely,1:nelz, [1: hole_width, nelx - hole_width + 1 : nelx]),[],1));

intersection = intersect(pasV,pasS);
pasS = setdiff(pasS, intersection);

fixed11 = 3*nodeNrs( [1:nely- hole_width +1] , 1, [1,nelx + 1])-2;
fixed12=3*nodeNrs( [1:nely- hole_width +1], 1, [1,nelx + 1])-1;
fixed13=3*nodeNrs( [1:nely- hole_width +1], 1, [1,nelx + 1]);
fixed14=3*nodeNrs( 1+nely, 1, [hole_width+1,nelx + 1 - hole_width])-2;
fixed15=3*nodeNrs( 1+nely, 1, [hole_width+1,nelx + 1 - hole_width])-1;
fixed16=3*nodeNrs( 1+nely, 1, [hole_width+1,nelx + 1 - hole_width]);
fixed = double(union(fixed16,union(fixed15,union(fixed14,union(fixed13,union(fixed11,fixed12))))));
% 
...
% direction3 = 3;
% g3 = 10.0;
lcDof1 = 3 * nodeNrs(1:nely-hole_width +1,nelz+1,1:nelx+1);
lcDof2 = 3 * nodeNrs(nely-hole_width+2:nely+1,nelz+1,hole_width +1:nelx- hole_width+1);
F(lcDof1) = 1.0;
F(lcDof2) = 1.0;
...
filename = strcat('xphys_double_sides_od_hole_',num2str(volfrac0),'_',num2str(loop),'.mat');
% Use the save function to save variables to a file
save(filename, 'xPhys','-v7.3');
filename1 = strcat('U_double_sides_od_hole_',num2str(volfrac0),'_',num2str(loop),'.mat');
% Use the save function to save variables to a file
...
savefig(strcat('QNC_double_sides_od_hole_',num2str(loop),'.fig'));
%end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
