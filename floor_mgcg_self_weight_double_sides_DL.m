...
floor_thickness_rabar = round(0.05 / 0.3 * nelz);
floor_thickness_cover = round(0.02 / 0.3 * nelz);
step_size = round(0.2 / 6.0 * nelx );
...
pasS3 = [];
x1 = round(0.05 / 6.0 * nelx);
while x1 < nelx
    if x1+step_size > nelx
        break;
    end
    pasS3 = union(pasS3, double(reshape(elmNrs(1:nely,[1: floor_thickness_rabar ,...
        nelz - floor_thickness_rabar+1 : nelz], x1+1: x1+floor_thickness_rabar),[],1)));
    x1 = x1 + step_size;
end
% pasS4 = [];
y2 = round(0.05 / 3.0 * nely);
while y2 <= nely
    if y2+step_size > nely
        break;
    end
    pasS3 = union(pasS3, double(reshape(elmNrs(y2+1 : y2+floor_thickness_rabar,[1 : floor_thickness_rabar , ...
        nelz - floor_thickness_rabar + 1:nelz ], 1:nelx),[],1)));
    y2 = y2 + step_size;
end

pasS = union(pasS3,union(pasS1, pasS2));
...
one_element_concrete = concrete_density * power((6.00 / nelx ),3);
dead = 300;
live = 400;
dead_load = 10.0 * (1.2 * dead * 6.0*3.0  / ((nelx+1) * (nely+1)) )/  one_element_concrete;
live_load_line = 10.0 * (1.4 * live * 6.0*3.0/ ((nelx+1) * (nely+1)) )/ one_element_concrete;
...
lcdof_line = 3 * nodeNrs(1:nely+1,nelz+1,1:nelx+1);

F0(lcdof_line) = dead_load + live_load_line;
...
filename = strcat('xphys_one_floor_DL',num2str(volfrac0),'_',num2str(loop),'.mat');
% Use the save function to save variables to a file
save(filename, 'xPhys','-v7.3');
filename1 = strcat('one_floor_U_DL_',num2str(volfrac0),'_',num2str(loop),'.mat');
% Use the save function to save variables to a file
...
savefig(strcat('QNC_Vone_floor_DL',num2str(loop),'.fig'));
%end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
